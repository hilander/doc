...
%licznik tabel
\newcounter{mgrtable}
\newcommand{\mgrcounter}{\addtocounter{mgrtable}{1} \arabic{mgrtable}}
\newcommand{\mgrLastCounter}{\arabic{mgrtable}}
\newcommand{\mgrNextCounter}{\addtocounter{mgrtable}{1}\arabic{mgrtable}\addtocounter{mgrtable}{-1}}
\setcounter{mgrtable}{0}
...
\indent
	...
	\end{tabular}\par
	\begin{small} Tabela \mgrcounter. Sekwencyjne wykonanie dwóch rozkazów \end{small} \par
	\end{center}\par
\par
%
\indent
...
\indent 
	...
	\end{tabular}\par
	\begin{small} Tabela \mgrcounter. Potokowe wykonanie rozkazów \end{small}
	\end{center}
...
\indent
	...
	Cykl zegara & 1 & 2 & 3 & \cellcolor{yellow} 4 & 5 & 6 & 7 \\ \hline
	\end{tabular}\par
	\begin{small} Tabela \mgrcounter. Potokowe wykonanie 10 rozkazów na jednostce superskalarnej\end{small}
	\end{center}
...
\indent
	...
	\end{tabular}
	\begin{small} Tabela \mgrcounter. Taksonomia Flynna \end{small}
	\end{center}
...
\indent
	...
	\end{tabular}\nopagebreak \\
	\begin{small} 
		Tabela \mgrcounter. Czas działania komponentów systemu komputerowego oraz proporcjonalny do niego czas
		reakcji człowieka.
	\end{small}
	\end{center}
	...
	elementów systemu komputerowego, przekładając te czasy na zbliżone do ludzkich miar. Jak można zauważyć w~tabeli \mgrLastCounter,
	...
	(nieblokującej, ang. \emph{asynchronous, non\dywiz{}blocking}) komunikacji pomiędzy procesorem a~urządzeniami zewnętznymi.
	Tabela \mgrLastCounter~pokazuje również celowość projektowania aplikacji tak, aby jak najczęściej używana była pamięć cache w~celu uniknięcia
...
\indent
	...
	\end{tabular}\\
	\begin{small} Tabela \mgrcounter. Klasyfikacja systemów operacyjnych \end{small}
	\end{center}
\par
%
...
\remark
{
	Tutaj znajdą się odwołania do dokumentów, które opisują wyniki badań problemu zarządzania wątkami w~modelu N:M.
}
\par
...
\remark
{
	Tutaj znajdą się informacje o~komponentach, które zostaną użyte w~implementacji wątków mieszanych, a~nie były
	opisane we wstępie, np.:
	\begin{itemize}
		\item getcontext/swapcontext;
		\item pthread;
		\item POSIX: sygnały, zdarzenia, dostęp do systemu plików;
		\item \ldots ?
	\end{itemize}
}
\par
...
\par
\subsection{Planista przestrzeni użytkownika: {\tt Scheduler\_us}, \emph{uss}}
\indent
\remark
{
	\begin{itemize}
