...
%
\indent
	Ze względu na dostęp do zasobów komputera, procesy są uruchamiane przez system operacyjny 
	w~dwóch warstwach (przestrzeniach), które z~punktu widzenia procesów są traktowane jak środowisko, w~którym działają: 
...
\indent
	...
	\begin{itemize}
		\item jądro monolityczne (ang. \emph{monolithic kernel}), które zawiera w~sobie większą część usług systemu. Wszystkie usługi jądra
			...
			Jądro systemu Linux posiada wiele cech jądra monolitycznego, dodatkowo udostępnia możliwość wprowadzania (oraz usuwania) rozszerzeń 
      (obsługa nowych urządzeń, itp.) w~trakcie działania,
      dlatego można mówić o~nim, że jest jądrem \emph{modularnym}.
...
\indent
  ...
  \begin{itemize}
    ...
    \item XFCE;
    \item odtwarzacz mutlimediów  (MPlayer);
...
\myownfigure{writen1000min}{writen1000min.png}{Wykres porównawczy dla \code{n}=[1..1000] i~\code{s}=1000 -- porównanie minimalnych czasów wykonania}{.65} 
\myownfigure{writen1000max}{writen1000max.png}{Wykres porównawczy dla \code{n}=[1..1000] i~\code{s}=1000 -- porównanie maksymalnych czasów wykonania}{.65} 
\myownfigure{writen1000avg}{writen1000avg.png}{Wykres porównawczy dla \code{n}=[1..1000] i~\code{s}=1000 -- porównanie uśrednionych czasów wykonania}{.65} 

\myownfigure{write100nmin}{write100nmin.png}{Wykres porównawczy dla \code{n}=[1..1000] i~\code{s}=1000 -- porównanie minimalnych czasów wykonania}{.65} 
\myownfigure{write100nmax}{write100nmax.png}{Wykres porównawczy dla \code{n}=[1..1000] i~\code{s}=1000 -- porównanie maksymalnych czasów wykonania}{.65} 
\myownfigure{write100navg}{write100navg.png}{Wykres porównawczy dla \code{n}=[1..1000] i~\code{s}=1000 -- porównanie uśrednionych czasów wykonania}{.65} 
\par
