...
\indent
  W~większości współczesnych systemów operacyjnych zrealizowano paradygmat \emph{programownia równoległego}.\\
  \emph{Zacytować wikipedię?:)}\\
  ...
  systemu).
  Wątek jest bytem zależnym od procesu, a~proces posiada wiadomości o~każdym wątku.
...
\indent
  ...
  tym pomyśle wymagają wsparcia jądra systemu operacyjnego, a~co za tym idzie, zmiany działania systemu operacyjnego.
  ...
  dlatego obecne wydania NetBSD dają możliwość wyboru systemu z~zaimplementowanym mechanizmem \emph{scheduler activations}, ale jedynie ,,na życzenie''.
  Można zauważyć, że każda nowa próba zaimplementowania mechanizmu włókien najczęściej jest oparta na scheduler activations, nawet 
  Implementacja włókien oparta na podobnym pomyśle jest oferowana obecnie także w~systemie Windows 7.
\par
%
\indent
...
\par
\indent
  Opisany powyżej sposób implementacji włókien
