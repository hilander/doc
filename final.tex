...
%
\indent
	System operacyjny dostarcza usługi w~następujących obszarach:
	\begin{itemize}
		\item rozwój aplikacji: OS dostarcza zestaw narzędzi do tworzenia i~rozwijania aplikacji użytkownika począwszy od
			edytorów po kompilatory, \emph{debuggery}, \emph{profilery}, itp.
...
\indent
	...
	system). OS musi zatem udostępniać pewną abstrakcję, w~ramach której pomieszczą się różnorodne wymagania dowolnej aplikacji użytkownika).
	Tą abstrakcją jest \emph{proces} (ang. \emph{process}).
\par
%
\indent
	System operacyjny można podzielić na dwie zasadnicze warstwy:
	\begin{enumerate}
		\item \emph{Jądro systemu} (ang. \emph{kernel}). Zawiera ono:
		\begin{itemize}
			\item warstwę abstrakcji sprzętu (ang. \emph{hardware abstraction layer, HAL}), w~której znajdują się m. in. sterowniki dla urządzeń 
				za szyną systemową.
			\item Moduł do zarządzania (szeregowania) zadań
			\item Moduł do zarządzania pamięcią wirtualną
			\item Moduły komuni
		\end{itemize}
		\item Warstwa użytkownika, w~której znajdują się aplikacje. Programy tej warstwy korzystają z~ułatwień i~narzędzi dostarczanych przez
			jądro systemu.
	\end{enumerate}
\par
%
