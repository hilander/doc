...
\indent
	\begin{figure}[h]
		...
		\centering
		\includegraphics[scale=.21,draft]{mixedscheme.png}
...
\label{sec:solution}
\indent
	Biblioteka dostarcza implementację wątków hybrydowych, jednakże bez konieczności dokonywania zmian w~kodzie jądra systemu operacyjnego.
	...
	co różni ją od tamtej implementacji, to jej skalowalność. 
\par
%
\subsection{Moduł \code{libcoro}: podstawowe operacje na włóknach}
\index{libcoro|texttt}
\label{sec:coroutine}
\indent
	Moduł \code{libcoro} biblioteki \code{libmixed} jest odpowiedzialny za:
	\begin{itemize}
		\item utworzenie włókna (koprocedury);
		\item inicjalizację włókna, przydzielenie mu kontekstu wykonania (stosu);
		\item uruchomienie w~przydzielonym kontekście;
		\item przechwytywanie sytuacji wyjątkowych;
		\item przechwycenie sterowania po wykonaniu przez włókno operacji \code{yield};
		\item obsługę zakończenia wykonywania włókna.
	\end{itemize}
	Klasa \code{libcoro::coroutine} zawiera podstawowe mechanizmy potrzebne do obsługi włókna.
	Każdy wątek tworzony przez użytkownika musi dziedziczyć po tej klasie. Dodatkowo, użytkownik musi
	przeciążyć metodę \code{run}, która zawiera kod uruchamiany po uruchomieniu wątku (wywołanie metody \code{start}).
	\begin{figure}[h]
		\centering
		\includegraphics[scale=.5,draft]{classlibcoroabstract.png}
		\caption{Zależności pomiędzy klasami włókien}
	\end{figure}
\par
