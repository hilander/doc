...
\indent
	...
	\begin{itemize}
		...
		\item Dostęp do systemu: pewne zasoby systemu mogą być używane jedynie przez jego własne komponenty, np. wspomniany wyżej bezpośredni
			...
			w~pamięci operacyjnej mogą być używane przez dokładnie jedną aplikację.
		...
		\item Księgowanie: śledzenie zachowania aplikacji, wytwarzanie bazy informacji, a~następnie ich analiza może dać efekt w~postaci
			...
			a~co za tym idzie, spowoduje szybsze działanie systemu.
	\end{itemize}
\par
%
\indent
	...
	rodzaju iluzję wyłączności dostępu do zasobów systemu.
	Tą abstrakcją jest \emph{proces} (ang. \emph{process}).
\par
%
\indent
	System operacyjny można podzielić na dwie zasadnicze warstwy, zwane również \emph{przestrzeniami}: \emph{przestrzeń jądra}
	(ang. \emph{kernel space}) oraz \emph{przestrzeń użytkownika} (ang. \emph{user space}).
\par
%
\indent
	...
	przydzielaniem czasu procesora poszczególnym programom. Instrukcje jądra wykonywane są w~przestrzeni jądra. Dzięki temu,
...
%
\indent
	Programy uruchamiane w~przestrzeni użytkownika posiadają limitowany dostęp do zasobów komputera:
	\begin{itemize}
		\item procesy . Co więcej, jej rzeczywiste położenie w~pamięci operacyjnej jest nieznane
			aplikacji, gdyż to jądro dokonuje translacji adresów fizycznych na adresy używane przez nie (również translacja w~drugą stronę
			przeprowadzana jest przez jądro).
			Również nie wszystkie rejestry procesora są dostępne dla aplikacji uruchomionej w~tej przestrzeni.
		\item 
	\end{itemize}
\par
%
...
%
\indent
	Ponieważ proces musi spełniać tak wiele ortogonalnych względem siebie wymagań, zdefiniowanie go jest dość trudne, gdyż granice tego pojęcia
	są dość rozmyte. Dobrym określeniem, które oddaje naturę procesu jest \emph{piaskownica} (ang. \emph{sandbox}), gdyż system wytwarza i~udostępnia 
	specjalnie dla niego:
	\begin{itemize}
		\item własną przestrzeń adresową, niezależną i~izolowaną od innych procesów;
		\item czas wykonania;
		\item uchwyty (ang. \emph{handle}) do zasobów systemowych;
		\item dostęp do mechanizmów komunikacji międzyprocesowej (ang. \emph{interprocess communication, IPC}).
	\end{itemize}
	Oprócz wyżej wymienionych zasobów, proces posiada także swój stan.
\par
%
\indent
